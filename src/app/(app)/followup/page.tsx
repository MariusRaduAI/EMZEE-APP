...
export default function FollowupPage() {
  ...
  const noDate = useMemo(() => db.clients.filter((c) => !c.event_date).sort((a, b) => a.couple.localeCompare(b.couple)), [db.clients]);

  const total = db.clients.length;
  const googleDone = db.clients.filter((c) => status[c.id]?.google).length;
  const feedbackDone = db.clients.filter((c) => status[c.id]?.feedback).length;
  ...
  return (
    <div className="fade-in">
      ...
      {total > 0 && (
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="stat">
            ...
            <p className="text-2xl font-bold tracking-tight text-green">{googleDone}<span className="text-lg text-faint">/{total}</span></p>
            <p className="text-xs text-faint">din toți clienții</p>
          </div>
          <div className="stat">
            <span className="text-xs text-muted">Feedback / testimonial</span>
            <p className="text-2xl font-bold tracking-tight text-brand-soft">{feedbackDone}<span className="text-lg text-faint">/{total}</span></p>
            <p className="text-xs text-faint">din toți clienții</p>
          </div>
        </div>
      )}
      ...
      {noDate.length > 0 && (
        <section>
          <h3 className="text-sm font-semibold text-muted uppercase tracking-wide mb-3">Fără dată de eveniment ({noDate.length})</h3>
          <div className="space-y-2.5">{noDate.map((c) => <EventRow key={c.id} c={c} st={status[c.id]} onToggle={toggle} />)}</div>
        </section>
      )}
      ...
      {total === 0 && (
        <div className="card p-12 text-center">
          ...
          <p className="font-semibold text-ink">Niciun client încă</p>
          <p className="text-[15px] text-muted mt-1">Adaugă clienți ca să le poți bifa recenzia și feedbackul aici.</p>
        </div>
      )}
    </div>
  );
    ...
    <div className="card p-3.5 flex flex-col sm:flex-row sm:items-center gap-3">
      <div className="flex items-center gap-3 min-w-0 flex-1">
        ...
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted truncate">{c.event_date ? fmtDate(c.event_date) : "Fără dată"}{c.venue ? ` · ${c.venue}` : ""}</p>
        </div>
      </div>
